...
def cosine_similarity(x, y):
    ...
    # calculate magnitude (length) of both vectors
    x_magnitude = la.norm(x)
    y_magnitude = la.norm(y)

    # handle division by zero: if either vector is zero, return 0
    if x_magnitude == 0 or y_magnitude == 0:
        return 0.0

    # do cosine similarity in [-1, 1]
...
def averaged_glove_embeddings_gdrive(sentence, word_index_dict, embeddings, model_type="50d"):
    ...
    # return zero vector if sentence is empty
    if not words:
        return embedding

    valid_word_count = 0

    # do loop through each word and accumulate embeddings
    for word in words:
        if word in word_index_dict:
            # Get word embedding via dictionary index and add to total
            embedding += embeddings[word_index_dict[word]]
            valid_word_count += 1

    # divide by valid word count if any valid words found
    if valid_word_count > 0:
        embedding = embedding / valid_word_count
...
def get_sorted_cosine_similarity(embeddings_metadata):
    ...
    if embeddings_metadata["embedding_model"] == "glove":
        ...
        # embed  input sentence embedding
        ...
        # embed each category word and compute similarity to the input embedding
        ...
        # ensure category embeddings exist in cache and compute and cache them if missing
        if cache_key not in st.session_state or not all(
                cat in st.session_state[cache_key] for cat in categories
        ):
            get_category_embeddings(embeddings_metadata)

        category_embeddings = st.session_state[cache_key]

        # embed input sentence embedding from OpenAI
        input_embedding = get_openai_embeddings(
            st.session_state.text_search,
    ...
    # sort the dictionary by descending similarity scores
    sorted_items = sorted(cosine_sim.items(), key=lambda x: x[1], reverse=True)
    # Return sorted list of tuples: [(category_index, similarity_score), ...]
    return sorted_items
